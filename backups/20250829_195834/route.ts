...
async function processPeriodData(
  ...
  for (const date of Array.from(datesFound)) {
...
async function processSingleDate(
  ...
  const responseData: any = {
    ...
    kpis: {
      ...
      topSeller,
      formulasProcessed: totalFormulas
    },
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Processar período
    const { processedReports, aggregatedUnmappedData } = await processPeriodData(diario, controle, formMap, session.user.id, checkOnly)
