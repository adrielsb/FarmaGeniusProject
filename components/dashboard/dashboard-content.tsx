...
import { EnhancedProductionView } from "@/components/production/enhanced-production-view"
...
export function DashboardContent({ user }: DashboardContentProps) {
  ...
  const [productionInsights, setProductionInsights] = useState<any>(null)
  ...
  const handleInteractiveMappingContinue = async (newMappings: Record<string, string>) => {
    ...
    try {
      ...
      if (user?.id) {
        const mappingsToSave = Object.entries(newMappings).map(([originalForm, mappedCategory]) => ({
          user_id: user.id!,
        ...
        await FormMappingsService.saveMappings(mappingsToSave)
        console.log('✅ Mapeamentos salvos no banco de dados')
      }
  ...
  const handleApplyMappings = async (newMappings: Record<string, string>) => {
    ...
    try {
      ...
      if (user?.id) {
        ...
        await FormMappingsService.saveMappings(mappingsToSave)
        console.log('✅ Mapeamentos manuais salvos no banco de dados')
      }
  ...
  const loadUserMappings = async () => {
    if (!user?.id) {
      console.log('ℹ️ Usuário não logado - carregamento de mapeamentos ignorado')
      return
    }
    
    try {
      console.log('🔄 Carregando mapeamentos do usuário:', user.id)
      const savedMappings = await FormMappingsService.getUserMappings(user.id)
      
      if (Object.keys(savedMappings).length > 0) {
        setMappingData(JSON.stringify(savedMappings, null, 2))
        console.log('✅ Mapeamentos carregados do banco de dados:', Object.keys(savedMappings).length, 'itens')
      } else {
        console.log('ℹ️ Nenhum mapeamento encontrado para o usuário')
      }
    } catch (error) {
      console.error('❌ Erro ao carregar mapeamentos salvos:', error)
      // Não interromper o carregamento da página por causa deste erro
    }
  }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          console.log('✅ Dados de produção carregados:', result.data.length, 'itens')
          setProductionData(result.data)
          setProductionInsights(result.insights)
        } else {
          console.log('⚠️ Nenhum dado de produção encontrado')
          setProductionData([])
          setProductionInsights(null)
        }
      } else {
        console.error('❌ Erro na API de produção:', response.status)
        setProductionData([])
        setProductionInsights(null)
      }
    } catch (error) {
      ...
      if (response.ok) {
        ...
        if (result?.success && result?.data) {
          ...
          try {
            ...
            const reportDate = result.data.report_date || ''
            ...
            // Automatically load the latest report data to show in table tab
            setTimeout(() => {
              autoLoadLatestReport()
            }, 500)
          } catch (dateError) {
  ...
  const loadHistoricalReport = async (reportId: string, reportDate: string, silent: boolean = false) => {
    if (!silent) {
      setStatus("idle")
      setStatusMessage("")
    }
  ...
  // Function to automatically load latest report (silent, no status messages)
  const autoLoadLatestReport = async () => {
    try {
      console.log('🔄 Auto-carregando último relatório...')
      ...
      if (historyData.success && historyData.reports && historyData.reports.length > 0) {
        ...
        console.log('📊 Auto-carregando relatório:', latestReport.id, 'Data:', latestReport.date)
        
        // Load the historical report data silently
        await loadHistoricalReport(latestReport.id, latestReport.date, true)
      } else {
        console.log('⚠️ Nenhum relatório encontrado para auto-carregamento')
      }
    } catch (error) {
      ...
      >
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="production">
            <EnhancedProductionView 
              data={productionData} 
              currentHour={new Date().getHours().toString() + ':00'}
            />
          </TabsContent>

          <TabsContent value="executive">
            <ExecutiveDashboard period={30} />
          </TabsContent>
        </Tabs>
      </motion.div>
